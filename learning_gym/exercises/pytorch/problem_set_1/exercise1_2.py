...
class MLPGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        """
        self.log_std = torch.nn.Parameter(-0.5 * torch.ones(act_dim))  # 按照the action vector初始化log_std，使之可训练
        self.mu_net = mlp([obs_dim] + list(hidden_sizes) + [act_dim], activation) # 生成网络
